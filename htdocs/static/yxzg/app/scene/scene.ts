...
export const setScale = (c,flags) => {
    ...
    scale = mgr_data.devicePixelRatio = devicePixelRatio;

    mgr_data.scale = devicePixelRatio;
...
export class mgr {
    ...
    static createScene(sceneData: any, name: string, cameraData: any, w: number, h: number, props?) {
        ...
        mgr.create( SceneCfg.defualCamera3D );
        mgr.create( SceneCfg.defualCamera2D );

        camera          = SceneCfg.defualCamera3D;
        camera2D        = SceneCfg.defualCamera2D;
        
        browserAdaptive();
        // div[mgr_data.name] = this.initDiv(width[mgr_data.name], height[mgr_data.name]);
        
        // if (props.useRenderTarget){
        //     mgr.createSceneRT();
        // }
    ...
    static modifyVisible ( data:any, visible: boolean, isRt? ){
        let impl = data._show.old.ref.impl
    ...
    static setOpacity( data: any, opacity: number, isRt? ) {
        ...
        if ( data.ref !== undefined ){
            data.meshRender.material.opacity = opacity;
...
let rootWidth, rootHeight, rootScale, wscale, hscale, rootX, rootY, scaleW, scaleH;
