...
// 兼容小游戏写法，var 声明的变量不会加到全局对象上
window.pi_modules = window.pi_modules || {};
// 定义基础函数模块
pi_modules.butil = {
	id: 'butil',
	exports: undefined,
...
pi_modules.store.exports = (function () {
	...
	module.write = function (store /*:Store*/, key /*:string*/, data /*:any*/, callback /*:function*/, errorCallback /*:function*/) {
        ...
        if (ArrayBuffer.isView(data)) {
            tx.objectStore(store.tabName).put(data.slice().buffer, key);
        } else {
...
pi_modules.load.exports = (function () {
	...
	// 检查是否是微信小游戏环境
	var inWXMiniGame = winit.isWXMiniGame;
  var nativePath = winit.nativePath;
	// 获得路径签名， "-"开头表示本地文件
		...
		store.init(localStore, function () {
			store.read(localStore, "", function (value) {
				if (value) {
					...
					ajax.get(nativePath+"/"+(inApp == 1?"depend":".depend"), undefined, undefined, ajax.RESP_TYPE_TEXT, 0, function (r) {
						localSign = {};
						var i = r.indexOf("["),
							j = r.lastIndexOf("]"),
							info;
	...
	var save = function (load, buff, files, size) {
		...
		for (i = count; i >= 0; i--) {
            ...
            // 使用视图切分数据，降低内存消耗
            data = new Uint8Array(buff, size-info.size, info.size);
			size -= info.size;
...
pi_modules.commonjs.exports = (function () {
		...
		while (len1 < len2) {
			for (i = len2 - 1; i >= len1; i--) {
                ...
                if (info.depend) {
                    arr = [];
                    if (info.depend.js) {
                        info.depend.js.forEach((js) => {
                            arr.push(js);
                        });
                    }
                    if (info.depend.tpl) {
                        info.depend.tpl.forEach((tpl) => {
                            arr.push(tpl);
                        });
                    }
                }
	...
	module.dataJS = function (arr, fileMap) {
		if (arr.length) {
			(module.debug) ? debugDefine(arr, fileMap) : releaseDefine(arr);
		}
	}
	/**
	...
	module.require = function (modNames, fileMap, successCallback, errorCallback, processCallback) {
		...
		down = load.create(needs, function (r) {
			...
			module.dataJS(arr, wait.fileMap);
	...
	module.words = [];
	var findCh = function(str) {
		str = str.replace(/(\s*(?<!\\)\/\/.*$)|(\s*(?<!\\)\/\*[\s\S]*?(?<!\\)\*\/)/mg,"");
		var ch = str.match(/[^\x00-\xff]/g)
		if (!ch) {
			return
		}
		for (let i = 0, len = ch.length; i < len; i++) {
			if (module.words.indexOf(ch[i]) >= 0) {
				continue
			}
			module.words.push(ch[i])
		}
	};
	// debug模式下，用node.src的方式加载模块
	var debugDefine = function (arr, fileMap) {
		for (var i = 0, e, mod; i < arr.length; i++) {
			e = arr[i];
			mod = pi_modules[module.modName(e.path)];
			if (mod.loaded || mod.buildFunc)
				continue;
			// findCh(butil.utf8Decode(fileMap[mod.info.path]));
			loadJS({
				mod: mod,
	...
	var buildForce = function (wait) {
		...
		for(let k in wait.fileMap){
			delete wait.fileMap[k];
		}
	...
	// ============================== 立即执行
	var isWXMiniGame = load.isWXMiniGame();
	// 定义全局的模块定义函数
	self._$define = isWXMiniGame ? function (str, func) {
        var mod = {};
        window.pi_modules[str] = { exports: mod };
