...
import { addJsonName, calNodeHashName, calTextHash, calTextHashName, chFuncName, installTextName, nextHashName } from '../util/gui_tpl';
import * as hash from '../util/hash';
import { logLevel, warn } from '../util/log';
import { reformClass, reformEvent } from './reform';
import { styleWithJsonForm } from './style';
...
export const gen = (syntax: Syntax) => {
    nodeIndex = 0;
    sid = 0;
    funcStrArr = [`let ${VNKeyWords._$temp}, ${VNKeyWords.node};`];
    funcStrIndex = 1;
...
const preorder = (syntax: Syntax, parent: Syntax) => {
    ...
    if (syntax && syntax.type === 'attr') {
        const { value } = syntax.token;
        if (value === 'style') {
            styleWithJsonForm(funcStrArr, index, funcStrIndex);
        } else if (value === 'class' || value === 'w-class') {
            // TODO: class || w-class
            reformClass(funcStrArr, index, funcStrIndex);
        } else if (value.startsWith('on-') || value.startsWith('ev-')) {
            reformEvent(funcStrArr, index, funcStrIndex);
        }
    }
    if (syntax.type === 'html' || syntax.type === 'jobj' || syntax.type === 'jarr' || syntax.type === 'jpair'
        || syntax.type === 'text' || syntax.type === 'jsexpr') {
...
const genTagSufFunc = (parent: Syntax): any => {// 返回值为一个函数	
    return (node: ParserNode) => {
        let str = ``;
        // 计算tag节点内容hash
        str += `${calNodeHashName}(${VNKeyWords.node});`;
        // 如果节点下没有脚本，其hash是一个今天的值
        if (node.cs) {
            str += `${chFuncName}(${VNKeyWords.node});`;
        }
        // nodeIndex > 1是，该元素一定有父节点
        if (nodeIndex > 1) {
            str += `${VNKeyWords._$parent}["${VNKeyWords.children}"].push(${VNKeyWords.node});}`;
        } else {
            str += `return ${VNKeyWords.node};}`;
        }
...
const gentTagPreFunc = (tagName: string, syntax: Syntax, id: number) => {
    return (node: ParserNode): String => {
        // const spanExtra = tagName === 'span' ? '"text": "", "textHash": 0,' : '';
        const spanExtra = tagName === 'span' ? `"${VNKeyWords.text}": "",` : '';
        const ssid = (<any>syntax).sid !== undefined ? (<any>syntax).sid : id;
        let str = `${VNKeyWords._$temp}=${VNKeyWords.node};{let ${VNKeyWords._$parent} = ${VNKeyWords._$temp};let ${VNKeyWords.node} = {${spanExtra}"${VNKeyWords.attrs}":{},"${VNKeyWords.event}":{},"${VNKeyWords.eventHash}":0,"${VNKeyWords.tagName}":"${tagName}","${VNKeyWords.sid}":${ssid}};`;
        if (tagName.indexOf('-') > 0 || tagName.indexOf('$') > 0 || tagName.indexOf('widget') >= 0) {// widget
            str += `${VNKeyWords.node}.${VNKeyWords.childHas} = false;${VNKeyWords.node}.${VNKeyWords.child} = null;`;
        } else {// 普通节点
            str += `${VNKeyWords.node}.${VNKeyWords.children}=[];`;
        }
        if (!node.cs && tagName !== 'span') {
            str += `${VNKeyWords.node}.${VNKeyWords.childHash} =${node.childHash};`;
        }
...
const textFunc = (syntax: Syntax, parent: Syntax) => {
    return {
        ...
        suf: (node: ParserNode): String => {
            ...
            let str = ``;
            if (syntax.value) {
                str = `${VNKeyWords._$temp}=${VNKeyWords.node};{let ${VNKeyWords._$parent} = ${VNKeyWords._$temp};let ${VNKeyWords.node} = ${installTextName}("${syntax.value}", ${node.hash}, ${VNKeyWords._$parent});`;
                str += `;
				`;
                if (parentIsWidget(syntax)) {
                    str += `${VNKeyWords._$parent}["${VNKeyWords.child}"] = ${VNKeyWords.node}["${VNKeyWords.text}"];${VNKeyWords._$parent}["${VNKeyWords.childHas}"] = true;`;
                } else {
                    if (nodeIndex > 1) {
                        str += `if (${VNKeyWords._$parent}["${VNKeyWords.tagName}"] !== 'span') ${VNKeyWords._$parent}["${VNKeyWords.children}"].push(${VNKeyWords.node});`;
                    } else {
                        str += `return ${VNKeyWords.node};`;
                    }
                }
...
const jarrFunc = (syntax: Syntax, parent: Syntax) => {
    return {
        ...
        pre: (): String => `${VNKeyWords._$temp}=${VNKeyWords.node};{let ${VNKeyWords._$parent} = ${VNKeyWords._$temp};let ${VNKeyWords.node} = [];`,
        suf: (node: ParserNode): String => {
            let str = ``;
            for (let i = 0; i < syntax.right.length; i++) {
                let v;
            ...
            if (!parent) {
                ...
                if (parent.type === 'jarr') {
                    str += `${VNKeyWords._$parent}.push(${VNKeyWords.node});}`;
                    ...
                    str += `${VNKeyWords._$parent}[${key}]= ${VNKeyWords.node};}`;
                } else if (parent.type === 'body') {
                    // parent分为真正有意义的节点和body节点
                    if (nodeIndex > 1) {
                        str += `${addJsonName}(${VNKeyWords.node}, ${VNKeyWords._$parent});`;
                    } else {
                        str += `return ${VNKeyWords.node};`;
                    }
                    str += `}`;
    ...
    return {
        ...
        pre: (node: ParserNode): String => {
            ...
            if (isBuildIn(syntax.right[1])) {
                ...
                str += `
				${VNKeyWords._$temp}=${VNKeyWords.node};{let ${VNKeyWords._$parent} = ${VNKeyWords._$temp};	`;
            }
        ...
        suf: (node: ParserNode): String => {
            ...
            if (!parent) {
                ...
                if (parent.type === 'jarr') {
                    ...
                    str += `${VNKeyWords._$parent}[${parent.right[0].right[0].value}]= ${VNKeyWords.node};}`;
                } else if (parent.type === 'body') {
                    // parent分为真正有意义的节点和body节点
                    if (nodeIndex > 1) {
                        str += `${addJsonName}(${VNKeyWords.node}, ${VNKeyWords._$parent});`;
        ...
        suf: (node: ParserNode): String => {
            ...
            if (p.type === 'attr') {
                ...
                isBuildIn(syntax.right[0]) ?
                    str = `${VNKeyWords.node}[${(<any>syntax).index}] = ${syntax.right[0].value};` : str = `${VNKeyWords.node}[${(<any>syntax).index}] = ${node.str};`;
            } else if ((p.type !== 'jpair') && node.str) {
                // 如果是jpair则会在返回的node中处理	
                str += `${VNKeyWords._$temp}=${VNKeyWords.node};{let ${VNKeyWords._$parent} = ${VNKeyWords._$temp};`;
                if (nodeIndex > 1) {
                    if (parentIsWidget(syntax)) {
                        str += `_addJson(${node.str}, ${VNKeyWords._$parent});`;
                    } else {
                        str += `_addText(${node.str}, ${VNKeyWords._$parent});`;
                    }
                } else {
                    str += `return {"${VNKeyWords.text}":${node.str}, "str":${node.str}, "${VNKeyWords.childHash}":${node.hash},"${VNKeyWords.childHas}":undefined};`;
                }
                str += '}';
            }
    ...
    return {
        ...
        pre: (node: ParserNode) => {
            ...
            for (let i = 0; i < syntax.right.length; i++) {
                ...
                if (name === 'w-class' || name === 'class') continue;
                if (name.startsWith('ev-') || name.startsWith('on-')) {
                    eventSize++;
                    continue;
                }
            ...
            if (size > 0) {
                str += `${VNKeyWords.node}["${VNKeyWords.attrSize}"] = ${size};`;
            }
            if (eventSize > 0) {
                str += `${VNKeyWords.node}["${VNKeyWords.eventSize}"] = ${eventSize};`;
            }
            str += `${VNKeyWords.node}["${VNKeyWords.attrHash}"] = ${node.hash};`;
...
const attrFunc = (syntax: Syntax, parent: Syntax) => {
    return {
        ...
        suf: (node: ParserNode): String => {
            ...
            const attr = (key === 'w-did') ? `["${VNKeyWords.did}"]` : `["${VNKeyWords.attrs}"]["${key}"]`;
            if (!syntax.right[1]) {
                return `${VNKeyWords.node}${attr} = ${key};`;
            }
            let str;
            ...
            if (type === 'identifier') {
                str = `${VNKeyWords.node}${attr} = ${value};`;
                ...
                str = `${VNKeyWords.node}${attr} = attrvalue;}`;// 属性是脚本
                if (key !== 'w-did') {
                    str += `${VNKeyWords.node}["${VNKeyWords.attrHash}"] = ${nextHashName}(${VNKeyWords.node}["${VNKeyWords.attrHash}"], ${calTextHashName}(${VNKeyWords.node}["${VNKeyWords.attrs}"]["${key }"]));`;
                }
            }

            // 父节点为body时，该属性移动是某个script的子孙节点
            if (syntax.parent.type === 'body') {
                str += `${VNKeyWords.node}["${VNKeyWords.attrHash}"] = ${nextHashName}(${VNKeyWords.node}["${VNKeyWords.attrHash}"], ${calTextHashName}(${value}));`;
            } else if (type === 'identifier' || type === 'attrStr' || type === 'singleattrStr') {
                node.hash = hash.nextHash(node.hash, calTextHash(value));// 计算属性值hash
            }
            if (key === 'w-tag') {
                str += `${VNKeyWords.node}["${VNKeyWords.tagName}"] = ${VNKeyWords.node}["${VNKeyWords.attrs}"]["${key}"];`;
            }
