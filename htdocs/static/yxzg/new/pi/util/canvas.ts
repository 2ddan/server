...
import { BlobType, loadError, loadOK, register, Res, RES_TYPE_BLOB, ResTab, BABYLON_IMAGE_TYPE } from '../util/res_mgr';
...
// imgtext的资源类型
export const RES_TYPE_GUI_IMGTEXT = 'guiimgtext';
// imgfliter的资源类型
export const RES_TYPE_GUI_IMGFILTER = 'guiimgfilter';
...
export const parseFont = (str: string): FontCfg => {
	...
	// iso 小游戏字体超大， https://stackoverflow.com/questions/51440257/safari-missing-font-weight-component-when-retrieving-font-property-from-canvas
	let weightStr = 'normal';
	if (cfg.weight < 400) {
		weightStr = 'lighter';
	}
	else if (cfg.weight < 700) {
		weightStr = 'normal';
	}
	else if (cfg.weight == 700) {
		weightStr = 'bold';
	...
	cfg.font = cfg.style + ' ' + weightStr + ' ' + cfg.size + 'px ' + cfg.face;
...
export const drawGuiImg = (img: HTMLImageElement): HTMLCanvasElement => {
...
// 图像模块的名称
const imgModName = "pi/util/img"; // butil.relativePath('./img', module.id);
...
// 创建ImgTextRes资源
const createGuiImgTextRes = (name: string, type: string, args: ImgTextCfg, funcArgs: any) => {
	task(() => {
		const text = drawText(args);
		const ab = text[1];
		const cfg = text[2];

		loadOK(name, type, cfg, Res, text[0]);
	}, undefined, 900, 0);
};
// 创建ImgFilterRes资源
const createGuiImgFilterRes = (name: string, type: string, args: ImgFilterCfg, resTab: ResTab) => {
	const file = args.file || butil.relativePath(args.img, args.path);
	resTab.load(BABYLON_IMAGE_TYPE + ':' + file, BABYLON_IMAGE_TYPE, file, resTab, (r) => {
		const img = r.link;
		task(() => {
			const canvas = document.createElement('canvas');
			const w = img.width,
			...
			request('calc', imgModName, 'guiFilter', [ctx.getImageData(0, 0, w, h), img.width, img.height, args.arr], [], 900, 0, (r) => {
				ctx.putImageData(r, 0, 0);
				loadOK(name, type, args, Res, canvas);
			}, (err) => {
				loadError(name, err);
			});
		}, undefined, 900, 0);
...
register(RES_TYPE_GUI_IMGTEXT, createGuiImgTextRes);
register(RES_TYPE_GUI_IMGFILTER, createGuiImgFilterRes);
