...
export const guiFilter = (data: ImageData, width: number, height: number, args: any[]) => {
	for (const f of args) {
		exports[f[0]](data.data, width, height, f[1], f[2], f[3]);
	}
	return data;
};
...
//=============================== PS 算法 =============================
/* @description 将图像转换成hsv格式，调整色相 饱和度 明度
* @param  {Uint8ClampedArray} data rgba格式的像素数组
* @param  {Number} width 图片的宽度
* @param  {Number} height 图片的高度
* @param  {Number} hue 色相变化值, [-180, 180]之间, 可选, 默认为 0
* @param  {Number} saturate 饱和度变化值, [-100,100], 可选, 默认为 0
* @param  {Number} lightness 亮度变化值,[-100,100] , 可选, 默认为 0
* @example
*/
export const hsv = (data: Uint8ClampedArray, width: number, height: number, hue?: number, saturate?: number, value?: number): void => {
	hue = hue || 0;
	saturate = saturate || 0;
	value = value || 0;
	for (let i = 0, n = data.length; i < n; i += 4) {
...
export const brightContrast = (data: Uint8ClampedArray, width: number, height: number, bright?: number, contrast?: number, threshold = 200): void => {
	contrast = contrast > 0 ? contrast * 1.5 : contrast;
	let cv = contrast <= -255 ? -1.0 : contrast / 255.0;
	if (contrast > 0 && contrast < 255)
		cv = 1.0 / (1.0 - cv) - 1.0;

	let values = new Array(256);
	for (let i = 0; i < 256; i++) {
		let v = contrast > 0 ? CheckValue(i + bright) : i;
		if (contrast >= 255)
			v = v >= threshold ? 255 : 0;
		else
			v = CheckValue(v + ((v - threshold) * cv + 0.5));
		values[i] = contrast < 0 ? CheckValue(v + bright) : v;
	}
	for (let i = 0, n = data.length; i < n; i += 4) {
		for (let j = 0; j < 3; j++) {
			data[i + j] = values[data[i + j]];
		}
	}
}
//=============================== END =============================
