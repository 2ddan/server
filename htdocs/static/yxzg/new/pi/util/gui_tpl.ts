...
// ============================== 导入的模块、类、函数、常量
import { styleStr2Json, styleStr2JsonName } from '../gui_compile/style';
import { VNKeyWords } from '../gui_virtual/virtaul_node_keyword';
import { commonjs } from '../lang/mod';
import * as hash from './hash';
import { logLevel, warn } from './log';
...
export const convertEntityName = '_a';
export const calAttrHashName = '_b';
export const calTextHashName = '_c';
export const calJsonHashName = '_d';
export const addJsonName = '_e';
export const installTextName = '_f';
export const addTextName = '_g';
export const chFuncName = '_h';
export const calNodeHashName = '_i';
export const nextHashName = '_j';
 ...
 * 计算json哈希
 */
export const calJsonHash = (data: any) => {
    let jsonHash = 0;
    Object.keys(data).forEach((k) => {
        const kHash = calTextHash(k);
        const vHash = calTextHash(data[k]);
...
export const calAttrHash = (node) => {
    let size = 0;
    let str = '';
    for (const key in node[VNKeyWords.attrs]) {
        if (key === 'w-tag') {
            node[VNKeyWords.tagName] = node[VNKeyWords.attrs]['w-tag'];
        } else if (key === 'w-did') {
            node[VNKeyWords.did] = node[VNKeyWords.attrs]['w-did'];
            delete node[VNKeyWords.attrs]['w-did'];
        } else {
            size++;
            str += key;
            str += node[VNKeyWords.attrs][key];
        }
    }
    node[VNKeyWords.attrSize] = size;

    return node[VNKeyWords.tagName] ? calTextHash(str) : (calTextHash(str) ^ calTextHash(node[VNKeyWords.tagName]));
};

export const addJson = (value, parent) => {
    if (typeof value === 'object') {
        parent[VNKeyWords.childHash] = hash.objHashCode(value, parent[VNKeyWords.childHash] ? parent[VNKeyWords.childHash] : 0, new Set<any>());
    } else {
        parent[VNKeyWords.childHash] = hash.anyHash(value, parent[VNKeyWords.childHash] ? parent[VNKeyWords.childHash] : 0, new Set<any>());
    }
    parent[VNKeyWords.child] = value;
    parent[VNKeyWords.childHas] = true;
};
export const addText = (value, parent) => {
    ...
    if (parent && parent[VNKeyWords.tagName] === 'span') {
        parent[VNKeyWords.text] = parent[VNKeyWords.text] ? parent[VNKeyWords.text] + value : value;
        parent.textHash = parent.textHash ? hash.nextHash(parent.textHash, hashv) : hashv;
    } else {
        parent[VNKeyWords.children].push({ [VNKeyWords.text]: value, str: value, [VNKeyWords.childHas]: hashv, hash: hashv, hasChild: undefined });
    }
};

export const installText = (value, hashv, parent) => {
    const node = {
        [VNKeyWords.text]: value,
        [VNKeyWords.childHash]: hashv,
        hash: hashv,
        str: value,
        hasChild: undefined
    };
    if (node[VNKeyWords.text].indexOf('&#x') > -1) {
        node[VNKeyWords.text] = node[VNKeyWords.text].replace(/&#x(\w+?);/g, (s0, s1) => {
            return String.fromCharCode(parseInt(s1, 16));
        });
    }
    node[VNKeyWords.text] = convertEntity(node[VNKeyWords.text]);

    if (parent && parent[VNKeyWords.tagName] === 'span') {
        parent[VNKeyWords.text] = parent[VNKeyWords.text] + node[VNKeyWords.text];
        parent.textHash = parent.textHash ? hash.nextHash(parent.textHash, hashv) : hashv;
    }
 ...
 */
export const calNodeHash = (node) => {
    const sidAndTagHash = hash.nextHash(calTextHash(node[VNKeyWords.tagName]), node[VNKeyWords.sid]);
    const attrsHash = hash.nextHash(node[VNKeyWords.attrHash] || 0, hash.nextHash(node[VNKeyWords.styleHash] || 0, node[VNKeyWords.eventHash]));
    let nodeHash = hash.nextHash(sidAndTagHash, attrsHash);
    if (node.textHash) {
        nodeHash = hash.nextHash(nodeHash, node.textHash);
    }
    node[VNKeyWords.nodeHash] = nodeHash;
    if (node[VNKeyWords.tagName] === 'span') { node[VNKeyWords.childHash] = 0; }
};

export const chFunc = (node) => {
    if (node[VNKeyWords.tagName] === 'span' || node[VNKeyWords.childHash]) {
        return;
    }
    node[VNKeyWords.childHash] = 0;
    if (node[VNKeyWords.children] && node[VNKeyWords.children].length > 0) {
        for (let i = 0; i < node[VNKeyWords.children].length; i++) {
            const child 		= node[VNKeyWords.children][i];
            const childNodeHash = child[VNKeyWords.nodeHash];
            const childHash 	= hash.nextHash(hash.nextHash(childNodeHash, child[VNKeyWords.childHash] || 0), i + 1);
            node[VNKeyWords.childHash] = hash.nextHash(node[VNKeyWords.childHash], childHash);
        }
    }
};
...
export const toFun = (s: string, path?: string) => {
    try {
        ...
        return (new Function(stringifyName, getName, hashName, pathName, convertEntityName, calAttrHashName,
            calTextHashName, addJsonName, installTextName, addTextName, chFuncName, calJsonHashName, styleStr2JsonName, 'return' + s))(toString, commonjs ?
                commonjs.relativeGet : null, hash, path, convertEntity, calAttrHash, calTextHash, addJson, installText, addText, chFunc, calJsonHash, styleStr2Json);
    } catch (e) {
        warn(level, 'tpl toFun, path: ' + path + ', s: ', s, e);
        throw (e);
    }
};
export const toFunComplete = (func: Function, path?: string) => {
    try {
        return func(toString, commonjs ? commonjs.relativeGet : null, hash, path, convertEntity, calAttrHash, calTextHash, addJson, installText, addText, chFunc, calJsonHash, styleStr2Json);
    } catch (e) {
...
export const getFunc = (path: string) => {
