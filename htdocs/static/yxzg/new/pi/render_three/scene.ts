...
class SceneImpl implements Scene {
	...
	constructor(renderer: Renderer, data: SceneData) {
		...
		this.sceneBg.add(new THREE.AmbientLight([1, 1, 1]));
		this.meshArray = [];
	...
	public render(deltaMS, isAutoClear) {
		...
		if(this.camera2D){
			...
			this.impl3D.render(this.sceneBg, this.camera2D, deltaMS);
	...
	public insert(obj, parent?, resTab?) {
		...
		const root = obj.attachment === '2D' ? this.scene2D : (obj.attachment == 'bg'? this.sceneBg : this.scene);
	...
	private initTextCon(textCon: Json, resTab: ResTab) {
		...
		let impl;
		if (textcfg.isCommon) {
			impl = new THREE.ImageText(textCon);
	...
	private initMeshAndMeshRender(meshRender: Json, geo: Json, resTab: ResTab) {
		...
		} else if (geo.type == "Custom"){
			impl = _LOAD.newloadCustom(this.renderer, geo, meshRender, resTab);
		} else {
