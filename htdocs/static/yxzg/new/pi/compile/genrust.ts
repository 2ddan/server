...
*/
export const gen = (mods: Map<string, Gendrust.RustMod>, depend: Array<string>, tree: Gendrust.ModTree): string => {
    let dropStr = "";
...
const gen_use = (typeCache: Map<string, {namespace: string, hash: number}>, uses: Array<string>, declarCreats: Array<string>) => {
    if(!typeCache){
        return ;
    }else{
...
const gen1 = (mods: Map<string, Gendrust.RustMod>, rObjs: Array<string>, uses: Array<string>, tree: Gendrust.ModTree): [string, string] => {
	let str1 = "",  str2 = "";
	mods.forEach((mod, k) => {
...
const gen2 = (mod: Gendrust.RustMod, rObjs: Array<string>, uses: Array<string>, tree: Gendrust.ModTree): [string, string] => {
    let str1 = "",  str2 = "";
    ...
    if(mod.classes){
        mod.classes.forEach((v, k) => {
            if((<any>v).rsflag === true || v.power !== "pub" || !Gendrust.isInclude( mod.getFullMod(v.name, false, tree), mod.cfg)){
                return;
            }
            let m = mod.cfg.default?"":mod.mod() + "::";
            let path = (tree.isMain?"":(tree.creatName + "::")) + m + Gendrust.typeToString(v as any);
...
const registerType = (type: Gendrust.Type, arr: Array<string>, mod: Gendrust.RustMod, structStr: string, tree: Gendrust.ModTree) => {
	if(!type){
		return;
	}
...
const registerParamType = (types: Array<Gendrust.Param>, arr: Array<string>, mod: Gendrust.RustMod, structStr: string, tree: Gendrust.ModTree) => {
	if(!types){
		return;
	}
	for(let i = 0; i < types.length; i++){
		registerType((<Gendrust.KeyType>types[i]).type || (<Type>types[i]), arr, mod, structStr, tree);
	}
}

const fnFunc = (func: Gendrust.GenFun, path: string, structName: string, h: number, mod: Gendrust.RustMod, tree: Gendrust.ModTree, vectoslice: boolean):[string, string]  => {
	let str = "", str1 = "", ps;
	if(func.sync || func.async){
		if(func.sync){
...
const regist_fun_meta = (hash: number, params: Array<Gendrust.Param>, result: Gendrust.Type, back?: Gendrust.Type, suf?: string): string => {
    let v = suf?1:0;
...
const tps_has_nobj = (params: Array<Gendrust.Param>, result: Gendrust.Type): boolean => {
...
const has_nobj = (type: Gendrust.Type): boolean => {
    if(!has_nobj){
        return false;
    }else if (Gendrust.isNativeObject(type.name)){
        return true;
...
const parseJSTypeParamFun = (name: string, type: Gendrust.Type, funcName: string, struct: string, r:Gendrust.RustMod, tree:Gendrust.ModTree, jsName?: string): string => {
	return tplFunc.parseJSTypePrama(null, name, type, funcName, struct, r, tree, Gendrust.deref, jsName );
}

const parseJSTypeResultFun = (name: string, type: Gendrust.Type, funcName: string, struct: string, r: Gendrust.RustMod, hasp: boolean, tree:Gendrust.ModTree, iq: boolean,jsName?: string): string => {
	return tplFunc.parseJSTypeResult(null, name, type, funcName, struct, r, hasp, tree, iq, jsName);
}

const dropTplFun = (hash: number, name: string): string => {
...
let tplFunc = {
	fnCallTpl: toFunc(compile(fnCallTpl, TplParser)),
	fnAsyncCallTpl: toFunc(compile(fnAsyncCallTpl, TplParser)),
	fnSyncCallTpl: toFunc(compile(fnSyncCallTpl, TplParser)),
	fnBodyTpl:toFunc(compile(fnBodyTpl, TplParser)),
	//registFnTpl:toFunc(compile(registFnTpl, TplParser)),
	implDerefTpl:toFunc(compile(implDerefTpl, TplParser)),
	registObjTpl:toFunc(compile(registObjTpl, TplParser)),
	parseJSTypePrama:toFunc(compile(parseJSTypePrama, TplParser)),
    parseJSTypeResult:toFunc(compile(parseJSTypeResult, TplParser)),
    dropTpl:toFunc(compile(dropTpl, TplParser)),
}