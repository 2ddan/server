...
import { VEventData } from './virtual_event';
import { VirtualContainerNode, VirtualNodeBase, VirtualWidgetNode } from './virtual_node';
 ...
 * @example
 */
export const rebindEventMap = (oldNode: VirtualContainerNode | VirtualWidgetNode, newNode: VirtualContainerNode | VirtualWidgetNode): void => {
    if (!oldNode.ext) {
        return;
    }
 ...
 * @example
 */
export const addUserEventListener = (node: VirtualNodeBase | VirtualWidgetNode, key: string, type: string, value: string): void => {
    let map = node.ext.eventMap;
    if (!map) {
        node.ext.eventMap = map = new Map();
    }
    if (value) {
        map.set(type, getUserEventListener(type, value));
    } else {
        map.delete(type);
    }
};
 ...
 * @example
 */
export const addNativeEventListener = (node: VirtualNodeBase | VirtualWidgetNode, realNode: HTMLElement,
 ...
 * @example
 */
export const addNativeEventListener2 = (node: VirtualNodeBase | VirtualWidgetNode, realNode: RElement,
    key: string, type: string, value: string): void => {
    const isCap = type === CAPTURE_EVENT_PRE;
    let map = node.ext.nativeEventMap;
    if (!map) {
 ...
 * @example
 */
export const notify = (node: VirtualContainerNode | VirtualWidgetNode, eventType: string, e: any): void => {
    ...
    if (e) {
        if (typeof e === 'object' || typeof e === 'function') {
            ...
            e.srcNode = node;
            // e.srcWidget为初始组件源
            e.srcWidget = node[VNKeyWords.widget];
            set = true;
        }
    } else {
        e = { type: eventType, srcNode: node, srcWidget: node[VNKeyWords.widget], node: null, widget: null };
    }
    while (node) {
        if (set) {
            // e.node为当前节点源
            e.node = node;
            e.widget = node[VNKeyWords.widget];
        }
        const listener = node.ext ? (node.ext.eventMap ? node.ext.eventMap.get(eventType) : null) : null;
        if (listener) {
            const r = listener(e, node[VNKeyWords.widget]);
        ...
        node = (node[VNKeyWords.parent]) ? node[VNKeyWords.parent] : node[VNKeyWords.widget].parentNode;
    }
};
...
const getNativeFunction = (h: Function): Handler => {
    ...
    const func = (e: VEventData): HandlerResult => {
        const node = (<any>func).vnode;
        // const node: VirtualNodeBase = (<RElement>e.current).virtual;
        if (!node) {
            return;
        }
        let ext = node[VNKeyWords.ext];
        if (!ext) {
            ext = node[VNKeyWords.ext] = {};
        }
        if (!ext.eventAttr) {
            ext.eventAttr = <any>{};
            ext.eventAttr[`${e.type}`] = { interval: e.type === 'pointerclick' ? TapInterval : 0, stop: TapStopPropagation, nextTime: 0 };
        }
        ...
        return h(e, node[VNKeyWords.widget]);
    };
 ...
 * @example
 */
const eventBind2 = (el: RElement, h: Function, name: string): EventBind => {
    // const func = gesture[commonjs.flags.mobile ? 0 : 1][name];
