...
import { isPowerOfTwo, nextPowerOfTwo } from "../../pi/util/math";
...
export class UINodeRenderCreator {
    ...
    private static createTextCon = ( node: UINode )=>{
        ...
        textCfg.space           = 0 ? undefined : node._t_space;
        textCfg.textAlign       = node._t_textAlign;
        textCfg.strokeColor     = 0 ? undefined : node._t_borderColor;
        textCfg.strokeWidth     = 0 ? undefined :  node._t_borderWidth;
        textCfg.isCommon        = node._t_isCommon;
        textCfg.width           = 0 ? undefined : node.width;
        textCfg.isPowerOfTwo    = isPowerOfTwo( node.width );
        textCfg.zoomfactor      = 1;

        textCon.show            = node.text;
        textCon.opacity         = node.opacity;
        textCon.textcfg         = textCfg;
        textCon.horizontalAlign = node._t_textAlign
