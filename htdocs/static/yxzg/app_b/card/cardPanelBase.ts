...
import { CardCenterCfg, CardData, CardCreatCfg, CardAnimFlag,
        CardCreatEndCfg, CardRemoveCfg, CardNormalCfg, CardExpendCfg } from "./cardBase";
...
export class CardDataFunc {
    ...
    private static computeStateCreat = ( cardData: CardData )=>{
        ...
        UINodeCtrl.updateNodeData( cardData.card, cardData.tempState );
    ...
    // 牌回收
    private static computeStateRemove = ( cardData: CardData )=>{
        let tempLeft: number, tempTop: number, tempScale: number, tempRotate: number;
        let tempTime: number, tempProgress: number;
        let tempStateDetail: UIDataStateDetail, tempState2Detail: UIDataStateDetail;
        ...
        tempLeft    = tempState2Detail.left
                        - ( tempState2Detail.left - CardRemoveCfg.Left ) 
                            * tempProgress;
        tempTop     = tempState2Detail.top
                        - ( tempState2Detail.top - CardRemoveCfg.Top ) 
                            * tempProgress;
        tempScale   = tempState2Detail.scale[0]
                        - ( tempState2Detail.scale[0] - CardRemoveCfg.Scale ) 
                            * tempProgress;
        tempRotate  = tempState2Detail.rotate[2]
                        - ( tempState2Detail.rotate[2] - CardRemoveCfg.Rotate ) 
                            * tempProgress;
    ...
    static updateCardZ = ( cardData: CardData, isCenter: boolean )=>{
        if ( isCenter === true ){
            UINodeCtrl.updateNodeData( cardData.card, new UIDataState( { "z_relat": CardCenterCfg.z_realt } ) );
        }else{
            UINodeCtrl.updateNodeData( cardData.card, new UIDataState( { "z_relat": cardData.z } ) );
        }
    }
    ...
    static updateState = ( cardData: CardData )=>{
        switch (cardData.animFlag) {
            ...
            case CardAnimFlag.SCRAP:{
                CardDataFunc.computeStateRemove( cardData );
                break;
            }
