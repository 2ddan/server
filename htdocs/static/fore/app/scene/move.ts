...
export class Move {
    ...
    static filter(type,e){
        ...
        if(type === "moveto"){
            ...
            Move.table[e.fighter.mapId] = JSON.parse(JSON.stringify(n));
            if(e.tid !== -1)
                initTargets(e,e.fighter.mapId);
            initFighter(Move.table[e.fighter.mapId].fighter,f,t);
        }else if(type === "remove"){
