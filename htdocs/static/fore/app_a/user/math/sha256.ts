...
*/
//pi.mod.define('pi.math.sha256', function (path) {
declare var exports;
...
function(h, s) {
	...
	u = v.Hex = {
		...
		parse: function(a) {
			for (var c = a.length,
			d = [], b = 0; b < c; b += 2) d[b >>> 3] |= parseInt(a.substr(b, 2), 16) << 24 - 4 * (b % 8);
	...
	k = v.Latin1 = {
		...
		parse: function(a) {
			for (var c = a.length,
			d = [], b = 0; b < c; b++) d[b >>> 2] |= (a.charCodeAt(b) & 255) << 24 - 8 * (b % 4);
	...
	t.Hasher = x.extend({
		...
		_createHmacHelper: function(a) {
			return function(c, d) {
				return (new (w as any).HMAC.init(a, d)).finalize(c)
			}
		}
	});
	var w = f.algo = {};
...
(function(h) {
	...
	f = f.SHA256 = g.extend({
		...
		_doProcessBlock: function(c, d) {
			...
			e = b[0], f = b[1], m = b[2], h = b[3], p = b[4], j = b[5], k = b[6], l = b[7], n = 0; 64 > n; n++) {
				if (16 > n) a[n] = c[d + n] | 0;
				else {
					var r = a[n - 15],
					g = a[n - 2];
