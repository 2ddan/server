<div maxId="20" test="test" style="position: absolute;width: 100%;height: 100%" w-sid="2">
    ...
    <div w-class="3" w-sid="3">
        ...
        <widget w-class="8" on-tap="{{it1.rank_bang ? 'gobackrank' : 'goback'}}" w-tag="app_a-widget-btn_pic-btn_pic" w-sid="8" style="z-index: 1;">
            {"guide":"returnTo","icon":"close"} 
        </widget>

        <widget w-class="10" w-tag="app_a-widget-pic_text-pic_text" w-sid="10">{"icon":"cover_title","width":187,"height":33,"align":"center","marginLeft":3,"text":"击杀记录","textCfg":"gangCoverTitle","space":0,"fontSize":21,"top":4,"left":0} 
        </widget>
        {{if it1.rank_bang}}
                    ...
                    {{for i,v of it1.initData.kill_info[it1.kill_info_index]}}
                        <div style="width:100%;height:50px;position:relative;">
                            {{for m,n of role_list}}
                                ...
                                <span style="width:22%;height:100%;line-height:50px;color:#fde7ca;font-size:18px;text-align:center;font-family:mnjsh;position: relative;display: inline-block;">{{Common.numberCarry(n[1].fight_power,1000000)}}</span>
