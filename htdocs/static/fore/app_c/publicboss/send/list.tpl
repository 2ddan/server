<div maxId="20" test="test" style="position: absolute;width: 100%;height: 100%" w-sid="2">
    ...
    <div w-class="3" w-sid="3">
        ...
        <widget w-class="10" w-tag="app_a-widget-pic_text-pic_text" w-sid="10">{"icon":"cover_title","width":187,"height":33,"align":"center","marginLeft":3,"text":"奖励详情","textCfg":"gangCoverTitle","space":0,"fontSize":21,"top":4,"left":0} 
        </widget>
            ...
            <div w-class="13" class="scroll_box_v" layout="scroll" w-sid="13" style="position: absolute;width: 450px;height: 97%;color: rgb(255, 255, 255);top: 6px;">
                {{if _award[5]}}
                    ...
                    <div style="width:100%;height:123px;position:relative;">
                        {{for j,k of it1.initData.role_info_list}}
                            {{let kill_gang_name = undefined}}
                            {{if _award[3] == n}}
                                {{: kill_gang_name = checkTypeof(k[1].gang_name,"Array") ? Common.fromCharCode(k[1].gang_name) : k[1].gang_name}}
                            {{end}}
                            {{if k[0] == n && n != _award[3]}}
                    
                                {{let imgX= Pi.pictures['playerhead'+k[1].career_id]}}
                                {{let name = checkTypeof(k[1].name,"Array") ? Common.fromCharCode(k[1].name) : k[1].name}}
                                {{let gangName = checkTypeof(k[1].gang_name,"Array") ? Common.fromCharCode(k[1].gang_name) : k[1].gang_name}}
                                <widget w-tag="app_a-widget-head-friend" style="position: absolute;width: 107px;height: 108px;top: 6px;left: 15px;">
                                    {"url":{{imgX}},"top":18.5,"level":{{k[1].level}},"width":107,"height":108}    
                                </widget>

                                <span style="font-family:mnjsh;font-size:22px;color:#fde7ca;position:absolute;left: 135px;top: 25px;">{{"S"+k[1].area+" "+name}}</span>
