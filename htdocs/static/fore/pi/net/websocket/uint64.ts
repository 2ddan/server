...
const endReverse = (result, bytes) => {
    var size = bytes.length, len, i, a, b, c, d;
    len = bytes.length / 4;
    for ( i = 0; i < len; i++) {
        a = i << 2;
        b = a + 1;
...
const add32 = (sum, x32, y32)=>{
    var lz = x32[1] + y32[1], hz, ln, hn, buf;
    ln = lz - INT32_MAX;
    if (ln <= 0) {
        ln = 0;
    } else {
...
let ONE = new Uint8Array(new ArrayBuffer(8), 0, 8);
ONE.set([0, 0, 0, 0, 0, 0, 0, 1]);
let x1 = new Uint8Array(new ArrayBuffer(8), 0, 8);
let x1_32=new Uint32Array(x1.buffer);
let y1 = new Uint8Array(new ArrayBuffer(8), 0, 8);
let y1_32=new Uint32Array(y1.buffer);
let z = new Uint8Array(new ArrayBuffer(8), 0, 8);
