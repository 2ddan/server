import {createU8ArrayView,createArrayBuffer,charToUtf8,utf8Length, utf8ToChar,
    isArray, isString, isNumber, isInt, isArrayBuffer} from "./util";
import {TAG, readInt8, readInt32, readString, decode as erlTermDecode} from "./erl_term";
import {deflateSync, inflateSync} from "./zlib";
import {encode as xxtea64Encode, decode as xxtea64Decode} from "./xxtea64";

const random =  (seed) => {
    var r = seed ^ 123459876;
    var temp:any = r / 127773;
    var s = 16807 * r - parseInt(temp, 10) * 2147483647;
    if (s < 0) {
        s += 2147483647;
    }
    return s;
...
const writeInt64 =  (jbuf, msg) => {
    var b, len = 7, h;
    b = createU8ArrayView(jbuf.buf, jbuf.offset, len);
    let temp:any = Math.abs(msg) / Math.pow(2, 32); 
    h = parseInt(temp, 10);
    b[6] = (h >>> 16) & 0xff;
    b[5] = (h >>> 8) & 0xff;
    b[4] = h & 0xff;
...
const mDecode =  (msg, jbuf, offset, length) => {
    var k, t, len = readInt8(readU8Array(jbuf, 1)), param = {}, dbuf:any, tempArray:any;
...
const _adler32 =  (data,adler) => {
    var MOD_ADLER = 65521, a = adler & 0xffff, b = (adler >>> 16) & 0xffff, index, adler;
...
const adler32 =  (data) => {
    var adler = 1,count = 32*1024- 1,offset=0;
...
const encode = (msg, encryption, cfg, isText = false) => {
    var i, k, v, len, cmd = msg.type, obj = msg.param, jbuf:any, adler, tJbuf, tValidBuf, eTValidBuf, validBuf, jbuf1:any, adlerBuf, version, index = 0;
    if (!isString(cmd)) {
        return 0;
    }
    jbuf = {};
...
const decode = (buf, encryption, num, isText = false) => {
    var i, msg, pos, pointer, size, len = buf.byteLength, i32array:any, alder, tAlder, version, jbuf1:any, tValidBuf, eTValidBuf, validBuf, tempOffset, cfg = {}, jbuf:any, tbuf, index = 0;
    if (len < 4) {
        return false;
    }
    jbuf = {};
