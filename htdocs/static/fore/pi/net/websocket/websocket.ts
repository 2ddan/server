...
import {getEncryption, nextEncryption, encode, decode} from "./protocol_block";
...
const send = (ws, msg, rId, sendType, cfg) => {
    //判定2进制数据
    var conMsg, key, temp:any = {}, encryption;
    //console.log(msg);
    if (sendType === "string") {
        conMsg = msg.type + "?=" + rId;
...
class PiWebSocket{
    ...
    private create(url, con, cfg){
        var ws:any = new WebSocket(url), openOk;
         ...
         */
        ws.onmessage =  (evt) => {
            var msg:any, b, preEncode, preDecode, newEncryption, checkMsg = function (iData) {
                var iData1, iData2, rm, i, iData3;
                iData1 = iData.split("?");
    ...
    open(url, cfg, timeout){
        let domain, i, port, handle, r, con:any = {}, rid = 1, rwait = {}, ws, isActive = false;
        con.getUrl =  () => {
			return url;
		};
		// 判断连接是否活动
