import {bsr64, bsl64, bxor64, band64, bnot64, sub64, add64} from "./uint64";

    

const le2be = function(bytes) {
    var len = bytes.length / 4, i, a, b, c, d, swp;
    for ( i = 0; i < len; i++) {
        a = i * 4;
        b = a + 1;
        c = b + 1;
...
const str2arr = function(str) {
    var len = str.length, arr = [], arr32, i, offset = 0;
    if (len >= 32) {
        for ( i = 0; i < 8; i++) {
...
const arrayCopy = function(src, start0, dest, start1, len) {
    var i, j;
...
const buf2arr8 = function(buf) {
    return new Uint8Array(buf, 0, buf.byteLength);
};

const get64 = function(arr8, bytes, offset) {
...
const set64 = function(bytes, offset, val) {
    var i, len = val.byteLength, index = offset * 8;
    for ( i = 0; i < len; i++) {
        bytes[index++] = val[i];
    }
};

const toUint64 = function(bytes) {
    return (bytes[0] << 56) | (bytes[1] << 48) | (bytes[2] << 40) | (bytes[3] << 32) | (bytes[4] << 24) | (bytes[5] << 16) | (bytes[6] << 8) | bytes[7];
};

const toArray64 = function(n) {
...
let BLOCK_LENGTH = 8;
let DELTA = buf2arr8(new ArrayBuffer(8));
DELTA.set([158, 55, 121, 185, 127, 74, 120, 0]);
let ZERO = buf2arr8(new ArrayBuffer(8));
ZERO.set([0, 0, 0, 0, 0, 0, 0, 0]);
let THREE = buf2arr8(new ArrayBuffer(8));
THREE.set([0, 0, 0, 0, 0, 0, 0, 3]);
let A = buf2arr8(new ArrayBuffer(8));
let B = buf2arr8(new ArrayBuffer(8));
let left0 = buf2arr8(new ArrayBuffer(8));
let left1 = buf2arr8(new ArrayBuffer(8));
let key = buf2arr8(new ArrayBuffer(8));
let count = 0;

const mul64 = function(n) {
    var sum = ZERO, i;
    for ( i = 0; i < n; i++) {
        add64(sum, sum, DELTA);
    }
...
const alignData = function(data) {
    var size = data.byteLength, arr8;
...
const alignData1 = function(size, len, data) {
    var copy, arr8,fill;
    switch(size) {
        case 1:
...
const mx = function(z, y, sum, key, p, e) {
    count++;
    var index;
    bsr64(A, z, 6);
    bsl64(B, y, 3);
...
const encode = function(rounds, key, data) {
    var t,k, v, n, swap = buf2arr8(new ArrayBuffer(8)), y = buf2arr8(new ArrayBuffer(8)), z = buf2arr8(new ArrayBuffer(8)), sum = buf2arr8(new ArrayBuffer(8)), p, e = buf2arr8(new ArrayBuffer(8)), arr, arr8, tmp;
    if ((rounds < 1) || (rounds > 32)) {
        throw ("invalid_rounds");
    }
	...
	/*解密数据*/
const decode = function(rounds, key, data) {
    var k, v, fill, len, n, swap = buf2arr8(new ArrayBuffer(8)), y = buf2arr8(new ArrayBuffer(8)), z = buf2arr8(new ArrayBuffer(8)), sum, p, e = buf2arr8(new ArrayBuffer(8)), arr, arr8, tmp;
    if ((rounds < 1) || (rounds > 32)) {
        throw ("invalid_rounds");
    }
    if (( typeof key) === "string") {
