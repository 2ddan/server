...
export class Polygon {
	...
	indexs: number[];          // 点在vertex数组的索引
	adjacency: number[];   // 邻接信息

	center: Vector3;    // 中心点，就是多边形所有顶点的坐标的平均值
	...
	// 计算包围盒
	private computeBox() {
		let maxX, minX, maxZ, minZ, vec = this.navMesh.vertexs[this.indexs[0]];
		maxX = minX = vec.x;
		maxZ = minZ = vec.z;
	...
	containsPoint(point: Vector3) {
		...
		let vec3 = new Vector3(), ret2 = new Vector3(), temp;
		for(let i = 1; i < this.indexs.length; i++){
...
export class LSegment implements Node{
	...
	p2:Vector3;
	center: Vector3;
	adjs:[number, number];//相邻多边形
	endPoint: SEPoint;
	navMesh:NavMesh;//相邻多边形
	cost:number;
	...
	// 迭代器：返回邻居
	*[Symbol.iterator]() {
		let adjsIndexs: Array<number>, poly: Polygon;
		if(this.endPoint)
			yield this.endPoint;
		else{
			for (let polyIndex of this.adjs) {
				poly = this.navMesh.polygons[polyIndex];
				adjsIndexs = poly.adjacency;
				for(let i of adjsIndexs){
...
export class NavMesh {
	vertexs:   Vector3[];   // 顶点的位置信息
	polygons: Polygon[];         // 多边形
	adjMap: Map<number, LSegment>// 相邻边
	//radio: number//目标半径
	 ...
	 */
	findPath(start: Vector3, end: Vector3, radio = 0) {
		let newEnd = new Vector3();
		let sNode = this.findNearest(start);
		let eNode = this.findNearest(end);
	...
	load(data: ArrayBuffer) {
		let i32 = new Int32Array(data, 16, 2);
		let iEnd = i32[0];//顶点结束索引 4字节
		let vEnd = i32[1];//顶点结束索引 4字节

		let vectorData = new Float32Array(data, iEnd, (vEnd - iEnd)/4);
		let vector: Vector3;
		let startIndex;
		...
		let faceData = new Int16Array(data, 24, (iEnd - 24)/2);
		let ploy: Polygon, indexs:Array<number>, adjPloy, adjPoint, side, cost, index, hash;
	...
	private findNearest(point: Vector3): SEPoint {
		...
		//点不在可行走区域时，执行以下代码	
		let min = Number.POSITIVE_INFINITY, tempDistance, ploy: Polygon;
		for(let p of this.polygons) {
		...
		let v1: Vector3, v2: Vector3, jionPoint: Vector3, temp1: Vector3, temp2: Vector3,n: Vector3, verticalP: Vector3,tempDist, shadowL, minDist = Number.POSITIVE_INFINITY;
	 ...
	 */
	private funnel(node: Node[], start: Vector3, end: Vector3, radio = 0) {
		...
		let oldV1: Vector3, oldV2: Vector3, newV1: Vector3, newV2: Vector3, radioV: Vector3;	
		let adjLine: LSegment;//相邻多边形共线
		let lastLine: Vector3;//lastPoint指向end的向量
		let maxIndex = node.length - 1,lastPoint = start, lastIndex1 = 0, lastIndex2 = 0;;
		points.push(start);
...
const getNext = (arr: Array<any>, i: number) => {
	if(i === arr.length - 1){
		return arr[0];
	}else{
		return arr[i + 1];
...
const getAdjLine = (navMesh: NavMesh, poly1: Polygon, poly2: Polygon): LSegment => {
	let max = Math.max(poly1.index, poly2.index);
	let min = Math.min(poly1.index, poly2.index);
...
//取到多边形中，有相邻多边形的边
const getAdjLines = (poly: Polygon): Array<LSegment> => {
	let lines = [];
...
const vIsOrder = (v1: Vector3, v2: Vector3, v3: Vector3): boolean => {
	let cross1 = new Vector3().crossVectors(v2, v1);
	let cross2 = new Vector3().crossVectors(v2, v3);
