...
import { VirtualNode, findNodeByAttr } from "../widget/virtual_node";
import { getRealNode, paintCmd3 } from "../widget/painter";
import { getSupportedProperty } from "../util/html";
import { calc, quadIn, cubicOut, sinOut } from "../math/tween";
import { TimerRef, setTimer, clearTimer } from "../util/task_mgr";
...
export class SlideTab extends Widget {
	transform: string = ""; // transform的兼容键
	end: number = 0; // 结束位置，为(卡片数量-1)*-100
	container: HTMLElement = undefined; // 卡片的容器
	containerWidth:number = 0;
	timerRef: TimerRef = undefined;
	startTime: number = 0; // 定时运动的起始时间
	startOffset: number = 0; // 定时运动的起始位置
	swipe: number = 0; // 挥动的方向，0为无挥动，-1左，1右
	lastOffset: number = 0;
	moving: number = 0;
...
const tween = (widget:SlideTab): any => {
	let d, time = now() - widget.startTime;
