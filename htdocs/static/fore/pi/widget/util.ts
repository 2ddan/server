...
export const releaseDefine = function (data/*:ArrayBuffer*/) {
    var blob = new Blob([data], { type: "text/javascript" });
    loadJS({src: URL.createObjectURL(blob), revokeURL: URL.revokeObjectURL });
};

export const loadJS = function (cfg) {
...
const loadNext = (suffixMap: Json, fileMap: Json, mods: Mod[], successCallback?: Function, processCallback?: Function): void => {
	...
	task(() => {
		...
		(loadTplMap as any).success = function(){
			successCallback && successCallback(fileMap, mods);
		}
		if(loadTplMap.length === 0){
...
const loadTpl1 = (file: string, fileMap: Json, widget: string): Json => {
	...
	if(!tpl){
		...
		tpl = {value:0,compile:0, path: s, wpath: null, init: function(){
			if(this.compile == 0){
				getCache(s);
			}
		}};
...
// ============================== 立即执行
self["$difineTPL"] = (name,func) => {
	// const s = butil.relativePath(file, widget);
	let tpl = getCache(name),idx = loadTplMap.indexOf(name);
	if (tpl.value == 0) {
		tpl.value = func
