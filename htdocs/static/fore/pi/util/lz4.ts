 ...
 * 编译asm.js命令
 * 
 * emcc --memory-init-file 0 --llvm-lto 1 --closure 1 -s EXPORTED_FUNCTIONS="['_LZ4_compress_default', '_LZ4_decompress_safe', '_LZ4_compressBound']" -O3 lz4.c lz4frame.c lz4hc.c xxhash.c -o lz4_asm.js
 * 
...
export const compress = (src: Uint8Array) => {
    ...
    let compressBound = LZ4_compressBound(src.length);

    let compressHeap = newU8Heap(compressBound);

    let compressSize = LZ4_compress_default(src, compressHeap.byteOffset, src.length, compressBound);
    if (compressSize <= 0) {
        // 压缩出错
...
export const decompress = (src: Uint8Array, decompressSize: number) => {
    ...
    let decompressHeap = newU8Heap(decompressSize);

    let realSize = LZ4_decompress_safe(src, decompressHeap.byteOffset, src.length, decompressSize);
    if (realSize <= 0) {
...
let LZ4_compress_default = LZ4ASM.cwrap('LZ4_compress_default', "number", ["array", "number", "number", "number"]);
...
let LZ4_decompress_safe = LZ4ASM.cwrap('LZ4_decompress_safe', "number", ["array", "number", "number", "number"]);
...
let LZ4_compressBound = LZ4ASM.cwrap('LZ4_compressBound', "number", ["number"]);
...
const newU8Heap = (size: number) => {
    let ptr = LZ4ASM._malloc(size);
    let r = new Uint8Array(LZ4ASM.HEAPU8.buffer, ptr, size);
    return r;
}