...
// export const FOR1 = "let _values = (o) => {return o.values ? o.values() : Object.keys(o)};\n";
// export const FOR2 = "let _entries = (o) => {return o.entries ? o.entries() : Object.keys(o).map(function(x) {return [x, o[x]]})};\n";
 ...
 */
export const compile = (s: string, construct: any, regex?: RegExp, regexIndex?: number, varname?: string, comment?: boolean, file?: string) => {
	let parser = new construct();
	parser.line = 0;
...
export const toFun = (s: string, path?: string) => {
	try{
	return (new Function(stringifyName, getName, hashName, pathName, convertEntityName, calAttrHashName, calTextHashName, addJsonName, installTextName,addTextName, chFuncName,"return" + s))(toString, commonjs ? commonjs.relativeGet:null, hash, path, convertEntity, calAttrHash, calTextHash,addJson, installText,addText, chFunc);
	}catch(e) {
...
const parseFor = (s: string, parser: Parser) => {
	...
	parser.putComment(s);
	let r = evaluate(arr[1]);
	s = arr[0].trim();
	arr = splitFirst(s, ",");
	let t = parser.getTempVar();
	if (arr) {
		parser.useFor2();
