 ...
 * @example
 */
export const objCall = (obj: any, func: string, args: Array<any>) => {
	if (Array.isArray(args)) {
		switch (args.length) {
			case 0:
				return obj[func]();
 ...
 * @example
 */
export const getValue = (obj: any, path: Array<string | number> | string, split?: string): any => {
	if (typeof path === 'string') {
		split = split || ".";
		let i = path.indexOf(split), j = 0;
		while (i > j) {
			let k = path.slice(j, i);
			let v = obj[k];
			if (v === undefined)
				return undefined;
			if (v === null)
 ...
 * @example
 */
export const setValue = (obj: any, path: Array<string | number> | string, value: any, split?: string): any => {
	let old;
	if (typeof path === 'string') {
		split = split || ".";
		let i = path.indexOf(split), j = 0;
	...
	let i = 0;
	for (let len = path.length - 1; i < len; i++) {
		let k = path[i];
		old = obj[k];
		if (!old) {
			// 如果键是数字
 ...
 * @example
 */
export const arrayEqual = <T>(oldArr: Array<T>, newArr: Array<T>): boolean => {
	if (oldArr === newArr)
		return true;
 ...
 * @example
 */
export const arrDrop = <T>(arr: Array<T>, el:T) : number => {
 ...
 */
export const arrSet = <T> (arr:Array<T>, el: T, i:number): Array<T> => {
	if(i < 0)
		return arr;
	if(i < arr.length) {
		const a = arr.slice();
 ...
 */
export const arrInsert = <T> (arr:Array<T>, el: T, i?:number): Array<T> => {
	const a = arr.slice();
	if(i === undefined)
		a.push(el);
	else if(i <= 0)
 ...
 */
export const arrDelete = <T> (arr:Array<T>, i:number): Array<T> => {
	if(i < 0)
		return arr;
	if(i === 0)
 ...
 * 删除元素
 */
export const arrRemove = <T> (arr:Array<T>, el: T): Array<T> => {
