...
export interface TaskInfo {
	func: any; // 任务的调用函数
	args: Array<any>; // 调用函数的参数（参数必须为数组或undefined）
...
export class TaskPool {
	sync : Map<number, Queue>;// 同步表，键为优先级
	syncSize : number;
	syncWeight : number;
	syncZero : Queue;
	async : Queue;
	 ...
	 * @example
	 */
	push (func:any, args:Array<any>, priority?:number, type?:TaskType):void {
		let t = taskCache.pop();
		if(t) {
			t.func = func;
			t.args = args;
	 ...
	 * @example
	 */
	pop (r:TaskInfo):boolean {
		let i, w = this.syncWeight;
	 ...
	 * @example
	 */
	getPrioritySize (priority:number, type?:TaskType):number {
		let queue = this.sync.get(priority);
	 ...
	 * @example
	 */
	clear (priority:number, type?:TaskType) : number {
		let queue = this.sync.get(priority);
		if(!queue)
			return 0;
		let size = queue.size;
...
// TODO 如果任务优先级很多，而且任务在池中大量堆积，需要优化权重算法，用SBTree树结构解决权重的快速命中。同步任务表可以用SBTree的修改版增加weight来使用，异步任务可以将SBTree改为WeightTree节点的key就是权重，size就是权重累加，根据权重累加来旋转
