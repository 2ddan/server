...
export class RSync {
	...
	//计算校验和
	checksum(data: Uint8Array): Array<Checksum> {
...
//序列化差异数据
export const encodeDiffs = (diffs: Array<Diff>, bb: BinBuffer) => {
	let diff;
...
//反序列化差异数据
export const decodeDiffs = (bb: BinBuffer): Array<Diff> => {
	let arr: Array<Diff> = [];
...
//以校验和的弱校验值为key，创建映射表
const createHashtable = (checksums: Array<Checksum>): Map<number, Array<Checksum>> => {
