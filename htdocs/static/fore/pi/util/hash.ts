 ...
 * @example
 */
export const arrHashCode = (arr: Array<any>, h: number, cycleObjSet: Set<any>): number => { //
	for (let v of arr) {
...
export const anyHash = (obj: any, h?: number, cycleObjSet?: Set<any>): number => { //
	...
	if (ArrayBuffer.isView(obj) && (<any>obj).BYTES_PER_ELEMENT > 0)
		return nextHash(nextHash(h || 0, 20 + (<any>obj).BYTES_PER_ELEMENT), getCrc32(new Uint8Array(obj.buffer, obj.byteOffset, obj.byteLength), 0, obj.byteLength));
	if (t === "function")
 ...
 * @param  {Uint8Array|Uint8ClampedArray} data 字节数组
 * @param  {Number} start 数组的起始位置
 * @param  {Number} end 计算的结束位置
 * @example
 */
 ...
 * CityHash 是后来Google受了Murmurhash3的启发，改进主要是由于更大的指令级并行性， Google内部的hash_map<string, int> 都是用这个算法了。CityHash这个有32位 64位 128 258输出等版本，还有使用SSE4.2的CRC32指令的版本。
...
export const murmurhash3_32 = (key: Uint8Array, seed: number): number => {
	...
	let c2 = 0x1b873593;
	let i = 0;
	for (let len = key.length - remainder; i < len; i += 4) {
		k =
			((key[i])) |
			((key[i + 1]) << 8) |
			((key[i + 2]) << 16) |
