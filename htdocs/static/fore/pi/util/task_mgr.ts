...
import { temp, TaskType, TaskPool } from './task_pool';
 ...
 * @param    {Function}    callback    回调函数, 参数的最后一位为TimerRef
 * @param    {any}        args    回调函数的参数
 * @param    {number}        ms    设置的时间间隔，单位：毫秒
 * @param    {number}    count    最多调用的次数, 0则不限次数, 默认为0
 * @param    {number}        delay    初始延迟时间，0表示为设置的时间间隔, 默认为0, 单位：毫秒
 * @return    {TimerRef}    定时器的引用
 */
export const setTimer = (callback: Function, args: Array<any>, ms: number, count?: number, delay?: number): TimerRef => {
	count = count || -1;
	let r = { ref: 0, count: count };
	let a: Array<any>;
 ...
 * @example
 */
export const set = (func: Function, args: Array<any>, priority: number, type: TaskType) => {
	if (func) {
...
// 函数调用计时
export const callTime = (func: Function, args: Array<any>, name: string, start?: number) => {
	start = start || timeNow();
	try {
		call(func, args);
	} catch (ex) {
