 ...
 * @example
 */
export const eleIndex = (i:any, arr:Array<any>, start:number, end:number, equal:Equal) : number => { //-1 | integer
	for(; start < end; start++) {
 ...
 * @example
 */
export const matchEqual = (arr1:Array<any>, offset1:number, arr2:Array<any>,offset2:number, len:number, equal:Equal) : boolean => { //true | false
 ...
 * @example
 */
export const arrayIndex = (arr1:Array<any>, start1:number, end1:number, arr2:Array<any>, start2:number, end2:number, equal:Equal, hash:GetHash) : number => { //-1 | integer
	// d是进制，m是防止溢出的素数，不同语言应该保证(d * (m + m) +d) 不应该溢出
	let i, h1, h2, pow = 1, d = 65537, m = 4294967291, len = end1 - start1;
	if(len > end2 - start2) {
		return -1;
 ...
 * @example
 */
export const matchPrefix = (arr1:Array<any>, start1:number, end1:number, arr2:Array<any>, start2:number, end2:number, equal:Equal) : number => { //integer
	let min, max, mid, offset;
 ...
 * @example
 */
export const matchSuffix = (arr1:Array<any>, start1:number, end1:number, arr2:Array<any>, start2:number, end2:number, equal:Equal) : number => { //integer
	let min, max, mid, offset;
 ...
 * @example
 */
const halfMatch1 = (arr1:Array<any>, start1:number, end1:number, arr2:Array<any>, start2:number, end2:number, equal:Equal, hash:GetHash, i:number, result:Array<any>, ri:number) : number => { //integer
	// 从指定的位置i开始，截取1/4的长度作为查询种子
	let prefixLen, suffixLen, s1, e1, s2, e2, bestLen = 0, end = i + Math.floor((end1 - start1) / 4), j = start2 - 1;
 ...
 * @example
 */
export const halfMatch = (arr1:Array<any>, start1:number, end1:number, arr2:Array<any>, start2:number, end2:number, equal:Equal, hash:GetHash, result:Array<any>) : boolean => { //true | false
	let j, len1 = end1 - start1, len2 = end2 - start2;
	if (len1 >= len2) {
...
// 定义的中间计算结果
const temp_r = [0,0,0,0,0,0,0,0], temp_v1 = [], temp_v2 = [];
/**
 * @description 用'Shortest Middle Snake'获得位置, 批分差异到前后两个部分并继续迭代
 * @example
 */
const  bisectSplit = (arr1:Array<any>, start1:number, end1:number, arr2:Array<any>, start2:number, end2:number, equal:Equal, hash:GetHash, result:Array<any>, x:number, y:number) => {
	temp_v1.length = 0;
	temp_v2.length = 0;
 ...
 * @example
 */
const  matchBisect = (arr1:Array<any>, start1:number, end1:number, arr2:Array<any>, start2:number, end2:number, equal:Equal, hash:GetHash, result:any) => {
	let x1, y1, x2, y2, k1_offset, k2_offset;
	// 缓冲长度以备多次调用
	let len1 = end1 - start1;
	let len2 = end2 - start2;
	let max_d = Math.ceil((len1 + len2) / 2);
	let v_offset = max_d;
 ...
 * @example
 */
const  matchCompute = (arr1:Array<any>, start1:number, end1:number, arr2:Array<any>, start2:number, end2:number, equal:Equal, hash:GetHash, result:Array<number>) => {
	let s1, e1, s2, e2;
 ...
 * @example
 */
export const  matchMain = (arr1:Array<any>, start1:number, end1:number, arr2:Array<any>, start2:number, end2:number, equal:Equal, hash:GetHash, result:Array<number>) => {
 ...
 * @example
 */
export const match = (arr1:Array<any>, arr2:Array<any>, equal:Equal, hash:GetHash, result:Array<number>) => {
	if (!arr1)
...
/**
 * @description 结构匹配， 单层的键值结构，键只能是字符串，值只能是基本类型。对该键值结构的模式匹配语法，支持与或非。如果值是数字字符串，则需要""引起来。值如果是字符串，支持用路径模式匹配语法来匹配。//以后可以支持变量提取，如果一个键值是需要匹配提取出来的，用[]，其他地方引用按照在$1来应用。还可以支持四则运算
 * @example
