...
const gentTagPreFunc = (tagName:string, syntax:Syntax, id: number) => {
	return (node:ParserNode):String=>{
		let ssid = (<any>syntax).sid !== undefined? (<any>syntax).sid: id;
...
const tagNodeFunc = (tagName:string, childs:Array<ParserNode>):ParserNode=>{
	let node = new ParserNode;
...
const jpairFunc = (syntax:Syntax, parent:Syntax)=>{
	return Object.assign({}, defaultParse,{
		...
		"pre":(node:ParserNode):String=>{
			...
			if(isBuildIn(syntax.right[1])){
				let v = parseBuildIn(syntax.right[1]);
				str += `
				node[${key}]=${v};`
...
//汇总静态hash
const calChildsHash = (childs:Array<ParserNode>, node: ParserNode) =>{
	for(let i = 0; i < childs.length; i ++){
	...
	return Object.assign({}, defaultParse,{
		"child":():Array<Syntax>=> {
			if(syntax.right[0].value === "w-sid"){
				(<any>syntax.parent.parent).sid = parseInt(syntax.right[1].right[0].value);
				return [];
			}
			let child = [];
		...
		"suf":(node:ParserNode):String=>{
			...
			let key = syntax.right[0].value, attr = (key === "w-did")? `.did`: `.attrs["${key}"]`;
			if(!syntax.right[1]){
				return `node${attr} = ${key};`
			}
			let str , type = syntax.right[1].type;
