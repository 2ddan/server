...
export const gen = (syntax:Syntax, path: string, cfg: Json) => {
	let arr = preorder(syntax).childs;
	let ret  = tplFunc(null,arr, path, cfg);
	return ret;
}
	...
	return Object.assign({}, defaultParse,{
		...
		"node":(childs: Array<ParserNode>): ParserNode => {
			...
			if(right[1] && right[1].type === "genType"){
				...
				node.members = childs.splice(1, childs.length) as Array<Member>;
			}else{
				node.members = childs as Array<Member>;
			}
...
const enumMemberFunc = (syntax:Syntax) => {
	return Object.assign({}, defaultParse,{
		"node":(childs: Array<ParserNode>): Member => {
			let right = syntax.right;
			let node = new Member;
			parseNote(syntax, node);//解析注释和注解
...
const genTypeFunc = (syntax:Syntax) => {
	return Object.assign({}, defaultParse,{
		"child":():Array<Syntax>=>{
			let arr = [], right = syntax.right;//type(不包含lifetime)
...
const importFunc = (syntax:Syntax) => {
	return Object.assign({}, defaultParse,{
		...
		"node": (childs: Array<ParserNode>): Import => {
			let node = new Import();
			node.type = "import";
			let paths = [],contents;
			for(let i = 0; i < childs.length; i++){
...
export const isInteger = (type: string) => {
	if(type === "i8" || type === "i16" || type === "i32" || type === "u8" || type === "u16" || type === "u64" ||  type === "isize" || type === "usize" ) return true;
}
...
export const isBase = (type: string) => {
	if(isNumber(type) || type === "bool") return true;
}
...
export const toFunc = (s: string) => {
	try{
	return (new Function("_stringify", "_tansType", "_typeToString", "_parseType","_isStruct","_isInteger", "_isString", "_createWBStr","_isBase", "_strHashCode","_upperFirst", "return " + s))(toString, tansType, typeToString, parseType, isStruct, isInteger, isString, createWBStr, isBase, hash.strHashCode, upperFirst);
	}catch(e) {
