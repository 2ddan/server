...
const getAlpha = (time: number, keys: GradientAlphaKey[]) => {
	...
	if (i === 0) {
		...
		let k = (time - keys[i - 1].time) / (keys[i].time - keys[i - 1].time);
...
class Gradient {
	...
	getValue(c: THREE.Color, time: number) {
...
class ColorImpl implements IMinMaxGradient {
	...
	getValue(c: THREE.Color, time: number, randoms?: number[]) {
...
class TwoGradientImpl implements IMinMaxGradient {
	...
	static min = new THREE.Color(0);
	static max = new THREE.Color(0);
	...
	getValue(c: THREE.Color, time: number, randoms?: number[]) {
		let min = TwoGradientImpl.min;
		let max = TwoGradientImpl.max;
