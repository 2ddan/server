...
let signMap = new Map<Function, Map<string, ParamSign[]>>();
...
export const registerSign = (constructor: Function, sign: Object) => {
	let map = new Map<string, ParamSign[]>();
	for (let methodName in sign) {
...
export class NativeObject {
	...
	private id = 0;                       // 底层对象对应的id，如果为0代表尚未初始化成功
	private state = NativeState.UnInit;   // 当前状态
	private waits = [];                   // 正在初始化时候，积累的函数；最后一个是方法名，其他是方法的参数

	/**
	 * 初始化方法，创建对象
	 * @param cb 监听器
	 */
	init(cb?: NativeListener) {
	 ...
	 * 调用底层静态方法
	 */
	static callStatic(constructor: any, methodName: string, params: any, id = 0) {
		...
		let args = [];
		let methodSign = signMap.get(constructor);
		let signs = methodSign.get(methodName);
 ...
 * 底层的回调
 */
window["handle_Native_Message"] = (cbID: number, code: number, ...args: any[]) => {

	// alert(`handle_Native_Message(${cbID}, ${code}, ${args.join(",")})`);

	if (cbID === 0) return;

	let cb = callIDMap.get(cbID);
	if (!cb) {
		return;
	}
...
window["handle_Native_ThrowError"] = (className: string, methodName: string, msg: string) => {
